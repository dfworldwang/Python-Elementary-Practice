...
class TreeNode:
    ...
    def __iter__(self):
    # we would like to simply iterate over all the keys in the tree in order.
...
class BinarySearchTree:
    ...
    # all we need to do is delete the node and remove the reference to this node in the parent.
    ...
    # What we need is a node that will preserve the binary search tree relationships
    # for both of the existing left and right subtrees.
    # The node that will do this is the node that has the next-largest key in the tree.
    # We call this node the successor,
         
            succ = currentNode.findSuccessor()
            succ.spliceOut()
            currentNode.key = succ.key
            curerntNode.payload = succ.payload

        else:
        # this node has one child
        # then we can simply promote the child to take the place of its parent
        # where the current node has a left child:
    ...
    # In this case we will just replace the key, payload, leftChild,
    # and rightChild data by calling the replaceNodeData method on the root.
